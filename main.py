...
async def get_code_from_gamesrader()->List[str]:
    try:
        async with aiohttp.ClientSession() as session, session.get('https://www.gamesradar.com/genshin-impact-codes-redeem/') as response:
...
async def get_code_from_programguide() -> List[str]:
    try:
        async with aiohttp.ClientSession() as session, session.get('https://progameguides.com/genshin-impact/genshin-impact-codes/') as response:
...
async def get_code_from_gipn() -> List[str]:
    async with aiohttp.ClientSession() as session, session.get('https://raw.githubusercontent.com/ataraxyaffliction/gipn-json/main/gipn-update.json') as response:
        json_content = await response.text()
...
async def get_code_from_pockettactics() -> List[dict]:
    codes = []
    try:
        async with aiohttp.ClientSession() as session, session.get('https://www.pockettactics.com/genshin-impact/codes') as response:
            html = await response.text()
